...
export function useRepairValidation() {
  ...
  const businessRules = {
    ...
    /**
     * 验证维修人员权限
     * @param repairer - 维修人员姓名
     * @returns 是否有权限
     */
    validateRepairerPermission: async (repairer: string): Promise<boolean> => {
      try {
   ...
   * @returns 验证结果
   */
  const validateField = (fieldName: string, value: any, formData: Partial<RepairRecordData> = {}): ValidationResult => {
   ...
   * @returns 表单验证结果
   */
  const validateForm = async (formData: Partial<RepairRecordData>): Promise<FormValidationResult> => {
    ...
    try {
      ...
      if (formData.device_number && formData.repair_date) {
        const isDuplicate = await businessRules.checkDuplicateRepair(formData.device_number, formData.repair_date)
        if (isDuplicate) {
   ...
   * @returns 警告信息数组
   */
  const checkDataIntegrity = (formData: Partial<RepairRecordData>): string[] => {
  ...
  const formatValidationErrors = (errors: Record<string, string>): string[] => {
    ...
    return Object.entries(errors).map(([field, errorMessage]) => {
