...
async def batch_delete_dict_types(
    ...
    try:
        from app.services.batch_delete_service import dict_type_batch_delete_service
        from tortoise.transactions import in_transaction
        ...
        async with in_transaction("default"):
            # 使用标准化批量删除服务
            result = await dict_type_batch_delete_service.batch_delete(ids=ids)
            
            # 生成用户友好的响应消息
            if result.failed_count == 0:
                message = f"成功删除 {result.deleted_count} 个字典类型"
                ...
                message = f"批量删除完成：成功删除 {result.deleted_count} 个，失败 {result.failed_count} 个字典类型。失败原因：{'; '.join(failed_details)}"
