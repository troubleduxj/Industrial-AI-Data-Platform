...
from .backends import StorageBackend, StorageResult, StorageBackendException
from .backends import LocalStorage, MinIOStorage
...
class ModelStorage:
    ...
    async def save(self, model_id: str, version: str, data: bytes) -> str:
        ...
        Returns:
            str: 存储路径
        """
        filename = f"model_{model_id}_v{version}.pkl"
        file_obj = io.BytesIO(data)
        result = await self._backend.upload(file_obj, filename)
        if result.success:
            return result.file_path
        raise StorageBackendException(result.error or "保存失败")
    ...
    async def load(self, path: str) -> bytes:
        ...
        Returns:
            bytes: 模型文件数据
        """
        file_obj = await self._backend.download(path)
